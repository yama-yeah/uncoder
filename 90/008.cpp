...
#define vvv1(type) vector<vector<vector<type>>>
...
#define Input(type, ...) \
    ...
    input(__VA_ARGS__)
#define scanv(a) \
    rep(i, sz(a)) { cin >> a.at(i); }
#define scanvv(a) \
    rep(i, sz(a)) rep(j, sz((a).at(i))) { cin >> a.at(i).at(j); }
#define Scanv(type, a, ...)    \
    vec(type, a, __VA_ARGS__); \
    scanv(a)
#define Scanvv(type, a, h, ...)  \
    vv(type, a, h, __VA_ARGS__); \
    scanvv(a)
#define outv(a)                               \
    rep(i, sz(a)) { cout << a.at(i) << " "; } \
    cout << endl
#define outvv(a)                                                \
    rep(i, sz(a))                                               \
    {                                                           \
        rep(j, sz((a).at(i))) { cout << a.at(i).at(j) << " "; } \
        cout << endl;                                           \
    }
template <class... T>
void input(T &...a)
...
template <class T, class... Ts>
void print15(const T &a, const Ts &...b)
{
...
} // make_permutations_list
template <class T, class T2>
void npk(T &a, T2 &b, int k)
{
    T2 c, d;
    nck(a, c, k);
    rep(typeof(c.at(0)) &i
...
int counts(string &s, string t)
{
    int c = 0, l = sz(s), tl = sz(t), r = 0;
    rep(i, l)
    {
        if (s[i] == t[c])
        {
...
vector<long long> sieve(int MAXP)
{
...
void solve()
{
    ...
    print(dp[N][atc.size()] % 1000000007);
}
int main()
{
