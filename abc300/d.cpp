...
vec(ll) sieve(ll MAXP){vec(ll) res;vector<int> mem(MAXP,0);for(int i=2;i<MAXP;i++){if(mem[i]){continue;}res.push_back(i);for(int j=i;j<MAXP;j+=i){mem[j]=1;}}return res;}
...
void solve()
{
    Input(ll,n);
    int ans=0;
    vec(ll) primes=sieve(100000);

    for (int i=0; primes[i]*primes[i]*primes[i]*primes[i]*primes[i] <= n; i++) {
        ll over=primes.size()+100;
        ll a=primes[i];
        for(int l=i+2;a*a*a*primes[l]*primes[l]<=n;l++){
            ll c=primes[l];
            for(int j=i+1;j<over&&primes[j]<c;j++){
                ll b=primes[j];
                if(a*a*b*c*c>n){
                    over=j;
                    break;
